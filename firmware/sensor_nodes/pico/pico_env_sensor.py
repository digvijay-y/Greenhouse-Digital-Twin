...
MQTT_CLIENT_ID = 'Pico1_Env_Sensor'
MOISTURE_TOPIC_1 = b"pico1/moisture1"
MOISTURE_TOPIC_2 = b"pico1/moisture2"
BME_TOPIC = b"pico1/bme280"
...
while True:
    try:
        ...
        bme_payload = "0.0,0.0,0.0" # Default value
        if bme:
            try:
                # The bme.values tuple contains ('24.7C', '1012.3hPa', '45.3%')
                temp_str, pres_str, hum_str = bme.values
                # Remove units ('C', 'hPa', '%') to send pure numbers
                temp = temp_str[:-1]
                pres = pres_str[:-3]
                hum = hum_str[:-1]
                bme_payload = f"{temp},{hum},{pres}"
            except Exception as e:
                print(f"Failed to read BME280: {e}")

        # 3. Publish to MQTT Broker
        print(f"Sending: M1={moisture_1}%, M2={moisture_2}%, BME={bme_payload}")
        
        client.publish(MOISTURE_TOPIC_1, str(moisture_1))
        client.publish(MOISTURE_TOPIC_2, str(moisture_2))
        client.publish(BME_TOPIC, bme_payload)
